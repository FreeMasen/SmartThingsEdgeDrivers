...
local json = (function()
  local s, module = pcall(require, 'st.json')
  if s then return module end
...
function OctoPi:get_printer()
  return assert(self:_get_request(__endpoints.printer))
end

function OctoPi:get_job()
  return assert(self:_get_request(__endpoints.job))
end
...
function OctoPi:gain_authorization()
  ...
  local body = json.encode({
    app = self.app_id or 'SmartThings Octopi Driver',
    user = self.user
  })
...
---if no api_key is present, call `gain_authorization`
---@return nil|integer @1 on success
---@return string|nil @nil on success
function OctoPi:_ensure_authorized()
...
function OctoPi:check_state()
  ...
  local authed, a_err = self:gain_authorization()
  if not authed then
    return nil, a_err
  end
