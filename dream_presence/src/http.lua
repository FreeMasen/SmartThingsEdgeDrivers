...
local function try_wrapped(sock, method, ...)
  local res = {sock[method](sock, ...)}
  if not res[1] then
    if method == "receive" and res[2] == "timeout" then
      local readers, _writers, err = socket.select({sock}, nil, sock.timeout)
      if not readers then
        return table.unpack(res)
      end
      return try_wrapped(sock, method, ...)
    end
    log.error(string.format("failed to call %q: %s", method, res[2]))
...
local function receiveheaders(sock, headers)
    ...
    if err then
      log.error("Failed to get first line:", err)
      return nil, err
    end
    ...
    while line ~= "" do
        ...
        if not (name and value) then
          log.warn("malformed response headers")
        ...
        line, err  = sock:receive()
        if err then
          log.error("error receiving headers:", err)
          return nil, err
        end
        ...
        while string.find(line, "^%s") do
            ...
            line = sock:receive()
            if err then
              log.error("error unfolding headers:", err)
              return nil, err
            end
        end
        -- save pair in table
...
socket.sourcet["http-chunked"] = function(sock, headers)
    return base.setmetatable({
        ...
        __call = function()
            ...
            if err then
              log.error("Error getting chunked size", err)
              return nil, err
            end
            local size = base.tonumber(string.gsub(line, ";.*", ""), 16)
            log.debug("size of chunk: ", size)
            if not size then return nil, "invalid chunk size" end
            -- was it the last chunk?
            if size > 0 then
                -- if not, get chunk and skip terminating CRLF
                local chunk, err, part = sock:receive(size)
                log.trace(string.format("received chunk: %q %q %q", chunk and #chunk, err, part))
...
function _M.open(host, port, create)
    ...
    try_wrapped(c, "settimeout", _M.TIMEOUT)
    try_wrapped(c, "connect", host, port)
...
function metat.__index:sendrequestline(method, uri)
    ...
    return try_wrapped(self.c, "send", reqline)
end
...
function metat.__index:receivestatusline()
    local status,ec = try_wrapped(self.c, "receive", 5)
    if not status then
    ...
    status = try_wrapped(self.c, "receive", "*l", status)
...
function metat.__index:receivebody(headers, sink, step)
    ...
    if t and t ~= "identity" then
      log.trace("transfer encoding: ", t)
      mode = "http-chunked"
    elseif base.tonumber(headers["content-length"]) then
      log.trace("by length: ", headers["content-length"])
      mode = "by-length"
    else
      log.trace("until closed")
    end
...
--[[local]] function trequest(reqt)
    ...
    if not code then
        log.warn("http/0.9 response maybe?")
