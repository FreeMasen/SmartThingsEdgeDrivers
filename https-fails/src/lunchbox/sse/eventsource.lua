...
--- @field private _sock_builder function optional. If this function exists, it will be called to create a new TCP socket on connection.
...
local function send_stream_start_request(payload, sock)
    print("send_stream_start_request", payload, sock)
    if not _sock_close then
        _sock_close = sock.close
        sock.close = function(...)
...
--- @param extra_headers table an optional table of key-value pairs (strings) to be added to the initial GET request
--- @param sock_builder function an optional function to be used to create the TCP socket for the stream. If nil, a set of defaults will be used to create a new TCP socket.
