...
---Fill the Headers table from incoming source
---@param self Request|Response
---@param key string|nil 'headers' or 'trailers', defaults to 'headers'
---@return string|nil
...
---Get the content-length header from `self`
---@param self Request|Response
---@return integer|nil
---@return string|nil
...
---Check if the header-value provided includes chunk encoding
---@param header string
---@return boolean
...
---@param self Request|Response
---@return {type: string, length: integer|nil, trailers: boolean|nil}|nil
---@return nil|string
function SharedLogic.body_type(self)
  local len, headers, enc, err
  len, err = SharedLogic.get_content_length(self)
  if not len and err ~= "no content header" then
    return nil, err
  end
  if len then
    return {
...
function SharedLogic.fill_chunked_body_step(self)
  ...
  if not trimmed then
    log.warn(string.format("Invalid len line found %q", len))
    return nil, "invalid length in chunk"
  else
  ...
  if not len2 then
    log.warn(string.format("Invalid length (not empty): %s", len))
    return nil, "invalid length in chunk"
  end
  log.warn(string.format("Reading %s byte chunk", len2))
  if not len2 then
  ...
  if not chunk then
    log.warn(string.format("Error reading %s: %q, (%q)", len2, err, partial))
...
function SharedLogic.fill_chunked_body(self)
  local ret, chunk, partial, err = "", nil, nil, nil
  repeat
    chunk, err, partial = SharedLogic.fill_chunked_body_step(self)
