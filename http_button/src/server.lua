...
local function broadcast_tick(sock, broadcast_ip, broadcast_port, url)
  local total_secs = 60
  local start = os.time()
  local last_ip
  while os.difftime(start, os.time()) < total_secs do
    local relative_to = math.min(0, total_secs - os.difftime(start, os.time()))
    sock:settimeout(relative_to)
    local bytes, ip_or_err, rport = sock:receivefrom()
    if ip_or_err == "timeout" or bytes == "whereareyou" then
      break
    elseif bytes == "whereareyou" then
      assert(sock:sendto(url, ip_or_err, rport))
    elseif not bytes then
      print("Error in multicast listener: ", ip_or_err, bytes)
      return
    elseif last_ip == ip_or_err then
      cosock.socket.sleep(1)
    end
    last_ip = ip_or_err
  end
  print("sending broadcast")
  assert(sock:sendto(url, broadcast_ip, broadcast_port))
end
...
cosock.spawn(function()
  while true do
    ...
    while true do
      local s, e = pcall(broadcast_tick, sock, ip, port, gen_url(server))
      if not s then
        break
      end
    end
