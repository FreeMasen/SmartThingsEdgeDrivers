...
function update_device_card(element, info, register_handlers) {
  ...
  if (current_class == EXTEND_CLASS) {
    ...
    if (register_handlers) {
      color_temp.addEventListener("change", () =>
        handle_change(device_id, PROP.COLOR_TEMP),
      );
    }
  }

  if (register_handlers) {
    temp_value.addEventListener("change", () =>
      handle_change(device_id, PROP.TEMP),
    );
    temp_c.addEventListener("change", () =>
      handle_change(device_id, PROP.TEMP),
    );
    air_value.addEventListener("change", () =>
      handle_change(device_id, PROP.AIR),
    );
    contact_open.parentElement.addEventListener("click", () =>
      handle_change(device_id, PROP.CONTACT),
    );
    contact_closed.parentElement.addEventListener("click", () =>
      handle_change(device_id, PROP.CONTACT),
    );
    switch_state_on.parentElement.addEventListener("click", () =>
      handle_change(device_id, PROP.SWITCH),
    );
    switch_state_off.parentElement.addEventListener("click", () =>
      handle_change(device_id, PROP.SWITCH),
    );
    switch_level.addEventListener("change", () =>
      handle_change(device_id, PROP.LEVEL),
    );
  }
}
...
function serialize_device_card(device_card, properties) {
  ...
  for (let prop of props) {
    switch (prop) {
      case "contact": {
        state.contact = get_binary_value(
          device_card,
          ".contact-open",
          "open",
          "closed",
...
function serialize_devices() {
  return Array.from(document.querySelectorAll(".device")).map((ele) =>
    serialize_device_card(ele),
  );
}
...
async function async_loop() {
  ...
  if (!device) {
    return console.warn("tried to start async loop w/o a device in the DOM");
  }
  let on_radio = device.querySelector(".switch-on");
  let off_radio = device.querySelector(".switch-off");
  let on = on_radio.checked;
  for (;;) {
    on = !on;
    if (on) {
      off_radio.parentElement.click();
    } else {
      on_radio.parentElement.click();
    }
    await Promise.sleep(1000);
  }
}
