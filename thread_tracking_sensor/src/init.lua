...
local function do_thread_metadata(driver)
    local meta = cosock.get_thread_metadata()
...
--     while true do
--         local msg, err = thread.gc_rx:receive()
--         if not msg then
--             print("gc_rx error", err)
--             break
--         end
--         print("gc_rx msg", msg)
--     end
...
local function do_memory_stuff()
    local json = require "st.json"
    if not memory then
        return
    end
    local lua_info = json.decode(memory.generate_report())
    local cosock_info = cosock.get_thread_metadata()
    local ttl_counts = 0
    local ttl_bytes = 0
    for k, v in pairs(lua_info) do
        if string.find(k, "count") then
            ttl_counts = ttl_counts + v
        end
        if string.find(k, "size") then
            ttl_bytes = ttl_bytes + v
        end
    end
    -- local lost = cosock.check_other_metadata_tables()
    -- for k, v in pairs(lost) do
    --     if v and #v > 0 then
    --         log.warn("cosock is holding thread open but it wasn't in threads", k, #v)
    --     end
    -- end
    print(utils.stringify_table({
        st_thread_count = thread.current_ttl_threads,
        cosock_thread_count = #cosock_info,
        lua_thread_count = lua_info.thread_count,
        lua_thread_size = lua_info.thread_size,
...
local memory_report_timer
memory_report_timer = driver:call_on_schedule(10, function(driver)
    if memory then
        do_memory_stuff()
    else
        print("memory is unavailable...")
        driver:cancel_timer(memory_report_timer)
    end
end, "memory-reporter")
